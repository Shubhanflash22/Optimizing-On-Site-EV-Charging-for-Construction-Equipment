...
#   * Single-Instance MPC Solver: a MILP over the shrinking prediction window.
...
# =============================================================================
#
# Faithful implementation of the full window model, specialized to a
# prediction window K_win that runs to the end of the day (shrinking horizon),
# started from the realized physical state. Because the window always reaches the
# true end of horizon, ALL original constraints apply exactly, including:
#   * terminal energy-neutral SOE (MCS & CEV end at their initial SOE),
#   * terminal position (MCS parked at a grid node),
...
function build_window_model(d, K_win, soe_mcs0, soe_cev0, mcs_node0, mcs_transit0,
    ...
    K = collect(K_win)                      # window interval indices
    Tb = vcat(K, last(K) + 1)               # window boundary indices (|K|+1)
    K_peak = [k for k in K if in_peak(k, delta_T, d.t_start)]
    is_terminal = last(K) == d.n_day        # window reaches the end of the daytime horizon (18:00)
    # productive_k[k] = true iff dig/load/travel are allowed in interval k (shift
    # hours, lunch EXCLUDED). Outside productive hours the CEV is still PRESENT on
    # site: it idles and may charge (there is NO overnight freeze in the daytime
    # horizon -- the day simply ends at 18:00 and the overnight is handled in Phase 2).
    ...
    # Terminal position (Eq. 10e), HARD when the window reaches 18:00: the MCS must
    # END the daytime horizon parked at a grid node, ready for the overnight Phase-2
    # recharge. The shrinking-horizon planner always reserves enough time to drive
    # home, so this is feasible every window.
    if is_terminal
    ...
    # ---- remaining work demand (closed-loop; past work captured by rem_*) ----
    ...
    # precedence: cumulative loading/swinging <= scale * cumulative digging
    # (hours form == original count form * delta_T; seeded with completed work).
...
# =============================================================================
#
# Shrinking horizon (accurate): at each interval k0 we solve the FULL remaining
# day [k0, nK] from the realized physical state, apply only k0's decisions,
# advance the realized state (incl. MCS in-transit carry-over and daily-peak
# carry-over), then move to k0+1 and re-solve. Because every solve reaches the
# true end of day, the terminal energy-neutral and end-at-grid constraints are
# always active, matching the original full-horizon optimum.
#
...
function run_scenario_1(; mode::Symbol = :synthetic,
    ...
    mcs_node = [first(d.N_g) for _ in d.M]              # MCS starts parked at grid
    mcs_transit = Any[nothing for _ in d.M]            # nothing or (i, j, remaining)
    rem_dig  = copy(float.(d.hours_digging))
    rem_load = copy(float.(d.hours_loading_swinging))
    cum_dig_e = zeros(length(d.E))                     # per-CEV cumulative hours done
    ...
    # One row per 15-min slot with only what a site worker needs: the time, what
    # each CEV should be doing, whether to plug each CEV in to charge, and whether
    # the MCS should be charging from the grid. All numbers/analytics live in the
    # detailed `closed_loop_trajectory.csv` for analysts instead.
    fe_time = String[]
    ...
    # ---- REPLANNING GRIDS ----------------------------------------------------
    # At each 15-min re-plan (row k0) the optimizer produces a FULL plan for every
    # remaining interval (columns). We store that plan here. Reading ACROSS a row shows
    # the whole forward plan made at that step; reading DOWN a column shows how the plan
    # for one interval is REVISED as new state + Bayesian-power information arrives. The
    # diagonal (k == k0) is what actually gets applied to the plant.
    plan_grid_kW = fill(NaN, nK, nK)                 # planned total MCS grid draw (kW)
    plan_mcs_soe = fill(NaN, nK, nK)                 # planned MCS-1 SOE at interval end (kWh)
    plan_cev_soe = [fill(NaN, nK, nK) for _ in d.E]  # planned CEV SOE at interval end (kWh)
    plan_cev_act = [fill("", nK, nK)  for _ in d.E]  # planned CEV activity name
    ...
    for k0 in 1:nK
        ...
        # NO FALLBACK: the window is solved ONLY with the HARD constraints
        # (CEV energy-neutral terminal 8b, precedence 12d, travel pacing 13). If a re-plan
        # from the realized state cannot satisfy them, nothing is relaxed -- that interval is
        # reported INFEASIBLE and the plant simply HOLDS state (no work / no charging) for it.
        cur_node = mcs_node[1]
        if !has_values(model)
            n_infeasible += 1
            @warn "No feasible solution at step k=$k0 under HARD constraints; holding state (no fallback)." status=termination_status(model)
            push!(log, (k0, clock_label(d, k0), d.lambda_whl_elec[k0], d.lambda_CO2[k0],
        ...
        # ---- apply ONLY the first interval's decisions ----
        ...
        cur_node = let nh = findfirst(i -> value(model[:z][1, i, k0]) > 0.5, d.N)
            nh === nothing ? 0 : nh                  # 0 = MCS in transit during k0
        end

        # ---- record THIS window's full forward plan into the replanning grids ----
        for k in K_win
            ...
            for e in d.E
                ...
                if site !== nothing
                    ...
                    plan_cev_act[e][k0, k] = sum(vals) < 0.5 ? "" : _ACT_NAME[d.B[argmax(vals)]]
                end
            end
        end
        ...
        # =========================================================
        # FEEDBACK STEP: each realized (possibly multi-activity) row
        # is a new regression observation that improves the estimate.
        # On real hardware: b = charging_received*dt - (SOC_next - SOC_now)*cap.
        # =========================================================
        for e in d.E
            row = a_real[e]
            if sum(row) > 1e-9
        ...
        # ---- advance realized MCS energy + position (incl. transit carry-over) ----
        for m in d.M
    ...
    # ---- KPIs from the realized closed-loop trajectory ----
    ...
    # Phase-1 MCS TOWING labour: rho_labor per hour the MCS spends in transit
    # (intervals where its parked node is 0 = in transit), summed over the day.
    transit_intervals = count(==(0), log.mcs_node)
    labour_cost  = d.rho_labor * d.delta_T * transit_intervals

    # ---- PHASE 2: overnight smart-charge restores each MCS to SOE_MCS_ini ----
    ...
    println("\n==== Scenario 1 closed-loop KPIs ====")
    println("-- Phase 1 (daytime 08:00-18:00) --")
    ...
    @printf("CEV SOE at 18:00    : %s kWh (target %s)\n",
            string(round.(soe_cev, digits = 2)), string(round.(d.SOE_CEV_ini, digits = 2)))
    println("-- Phase 2 (overnight 18:00-08:00 smart-charge) --")
    @printf("MCS SOE at 18:00    : %s kWh\n", string(round.(soe_mcs, digits = 2)))
    @printf("Overnight recharge  : %.2f kWh grid  ->  \$%.2f (cheapest hours)\n",
            overnight_energy, overnight_cost)
    @printf("MCS SOE at 08:00    : %s kWh (target %s, energy-neutral)\n",
            string([round(ov_df[end, Symbol("MCS$(m)_soe_kWh")], digits = 2) for m in d.M]),
            string(round.(d.SOE_MCS_ini, digits = 2)))

    # ---- export ----
    mkpath(out_dir)
    # (a) detailed ANALYST trajectory: powers, SOE, prices, CO2, estimates, etc.
    CSV.write(joinpath(out_dir, "closed_loop_trajectory.csv"), log)
    # (a2) overnight MCS smart-charge schedule (Phase 2).
    CSV.write(joinpath(out_dir, "overnight_mcs_charge.csv"), ov_df)
    # (b) simple WORKER schedule: only time + per-CEV activity + plug-in flags +
    #     MCS grid-charge flag. This is the CSV shown to site workers.
    fe = DataFrame(time = fe_time)
    for e in d.E
        fe[!, Symbol("CEV$(e)_activity")]       = fe_act[e]
        fe[!, Symbol("CEV$(e)_plug_in_charge")] = fe_chg[e]
    end
    fe[!, :MCS_charge_from_grid] = fe_mcs
    CSV.write(joinpath(out_dir, "worker_schedule.csv"), fe)
    # (c) REPLANNING GRIDS: rows = re-plan step (clock @ k0), cols = interval clock,
    #     cell = the value the optimizer PLANNED for that interval at that re-plan.
    grid_dir = joinpath(out_dir, "replan_grids"); mkpath(grid_dir)
    write_replan_grid(joinpath(grid_dir, "plan_grid_kW.csv"), plan_grid_kW, d, nK)
    write_replan_grid(joinpath(grid_dir, "plan_mcs_soe.csv"), plan_mcs_soe, d, nK)
    for e in d.E
        write_replan_grid(joinpath(grid_dir, "plan_cev$(e)_soe.csv"),      plan_cev_soe[e], d, nK)
        write_replan_grid(joinpath(grid_dir, "plan_cev$(e)_activity.csv"), plan_cev_act[e], d, nK)
    end
    make_plots(d, log, out_dir)
    println("\nResults written to: $(abspath(out_dir))")
    println("  - worker_schedule.csv         (simple, for site workers)")
    println("  - closed_loop_trajectory.csv  (detailed daytime, for analysts)")
    println("  - overnight_mcs_charge.csv    (Phase 2 overnight smart-charge)")
    println("  - replan_grids/*.csv          (per-step forward plans + replanning)")
    return log
end
