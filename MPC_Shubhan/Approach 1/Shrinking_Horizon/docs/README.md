...
realized energy consumed. The day is solved in **two phases**: a **daytime MPC**
(08:00–18:00) re-solved every **15 min**, then a deterministic **overnight
smart-charge** (18:00–08:00) that refills the MCS in the cheapest hours.
...
| `shrinking` | `true` | **accurate** shrinking horizon (solve to end of daytime); set `false` for a fixed lookahead `H` |
| `H` | 16 | fixed lookahead length (only when not shrinking) |
...
|------|----------|----------|
| `worker_schedule.csv` | **Site workers** | Dead-simple, one row per 15 min of the **daytime** plan: `time`, each CEV's `..._activity` (Digging / Loading/Swinging / Traveling / Idle), each CEV's `..._plug_in_charge` (Yes/No), and `MCS_charge_from_grid` (Yes/No). Nothing else. |
| `closed_loop_trajectory.csv` | **Analysts** | Everything for the daytime (08:00–18:00): grid/discharge/work power, SOEs, MCS node, time-of-use price, CO₂, **power estimates** `est_*`, uncertainty `unc_*`, observation count `n_obs` |
| `overnight_mcs_charge.csv` | **Analysts** | **Phase 2** overnight (18:00–08:00) MCS smart-charge: per-interval price, charge power, SOE, and a charging flag — the cheapest-hours refill back to the MCS start level. |
| `replan_grids/*.csv` | **Analysts** | Per-step **forward plans**: one file each for planned grid draw, MCS SOE, and per-CEV SOE/activity. Rows = the 15-min re-plan step; columns = interval. Reading **across a row** = the whole forward plan made at that step; reading **down a column** = how the plan for one interval is **revised** as new state + Bayesian info arrive (the diagonal is what actually gets applied). |
...
| 2. Window MILP (`build_window_model` / `build_and_solve_window`) | **Faithful implementation** of the full window model over the remaining horizon |
| 3. MPC loop (`realized_activity_durations`, `advance_mcs_state`, `run_scenario_1`) | Shrinking-horizon loop with state + parameter feedback |
...
- **Phase 1 — daytime MPC (08:00–18:00, 40 intervals):** a **shrinking horizon**
  re-solves the *entire remaining daytime* `[k0, nK]` from the realized state each
  15 min. Productive work (dig/load/travel) happens 08:00–12:00 and 14:00–17:00;
  the 12:00–14:00 lunch and 17:00–18:00 wind-down are non-productive but the CEV
  stays on-site and **may charge**. Each CEV must end at its **start-of-day level**
  (`soe_ini`, full in the main case) — energy neutrality (8b), a **hard** constraint
  `SOE_end ≥ soe_ini − term_tol` (`term_tol = 0` is the exact equality; the small
  default `0.1 kWh` absorbs estimator drift so the re-plan stays feasible). The MCS must be
  **parked at a grid node by 18:00** (hard, 10e), and draws from the grid during the day
  **only when needed** to stay above its 20% floor.
- **Phase 2 — overnight smart-charge (18:00–08:00):** deterministic, not an MILP.
  The MCS (now at the grid) is refilled back to `soe_ini` in the **cheapest** 15-min
  slots — closing its 24 h energy-neutral cycle. Output: `overnight_mcs_charge.csv`.

Both batteries are therefore **energy-neutral** over the full cycle (end = start);
on the main day both start full, so they end full.
