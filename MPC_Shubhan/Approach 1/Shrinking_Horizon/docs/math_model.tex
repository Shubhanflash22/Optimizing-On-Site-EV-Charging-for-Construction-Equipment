...
\begin{equation}
W(k)-w_{\text{pt}}\ \le\ w_{\text{pt}}\,V(k)\ \le\ W(k) + \tau,
\end{equation}
seeded with the travel/work already applied in earlier windows, where $\tau =
\texttt{pacing\_tol} = 0.05$ (interval-units) absorbs the sub-interval rounding residue that the
energy-capping guard above can leave in \texttt{cum\_dig\_e}/\texttt{cum\_load\_e} — without $\tau$,
that residue can strand the two-sided band on either side of an integer boundary with no reachable
whole-interval solution in between, making a schedule spuriously infeasible for reasons unrelated
to the pacing rule's intent. $\tau$ is small relative to one full interval (1.0) and is not enough
to let a CEV travel a genuine interval "early."
