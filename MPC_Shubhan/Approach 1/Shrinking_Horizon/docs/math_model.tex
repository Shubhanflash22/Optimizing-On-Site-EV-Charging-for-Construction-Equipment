...
\paragraph{State guard.} The realized CEV balance is clamped to
$[\underline{SOE}_e,\overline{SOE}_e]$. The clamp is a numerical guard, not physics: when it
binds, energy is created (low side) or discarded (high side) and the reported SOE trajectory
ceases to be the integral of the reported power. Clamp events are counted and reported per run,
because they are invisible to the MILP's own feasibility status.
