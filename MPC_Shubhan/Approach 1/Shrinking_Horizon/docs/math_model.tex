...
the one 24\,h MILP. The activity powers $p_a$ are the mean of a fixed calibrated estimate
$\mathcal{N}(\mu,\sigma)$; the MILP plans on $\mu$, and the closed-loop plant draws each
interval's realized power from $\mathcal{N}(\mu,\sigma)$ (truncated at $0$, idle pinned to $0$).
The model is not re-fit online.
