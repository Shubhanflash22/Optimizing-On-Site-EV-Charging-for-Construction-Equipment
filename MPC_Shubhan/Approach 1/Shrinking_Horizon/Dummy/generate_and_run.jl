# #############################################################################
# Dummy stress-test harness for the Shrinking-Horizon MPC.
# Builds N dummy :input datasets (1 MCS, any number of CEVs), runs the closed
# loop on each, writes per-case input+output under Dummy/<case>/, and compiles
# one comprehensive comparison table -> Dummy/summary_table.csv (+ .md).
# #############################################################################
...
function html_outputs(res, outdir)
    d = res.d
    gdir = joinpath(outdir, "replan_grids"); mkpath(gdir)
    Output._write_replan_grid_html(joinpath(gdir, "plan_grid_kW.html"), res.plan_grid_kW, res)
    Output._write_replan_grid_html(joinpath(gdir, "plan_mcs_soe.html"), res.plan_mcs_soe, res)
    Output._write_replan_grid_html(joinpath(gdir, "plan_mcs_activity.html"), res.plan_mcs_act, res)
    for e in d.E
        Output._write_replan_grid_html(joinpath(gdir, "plan_cev$(e)_soe.html"),      res.plan_cev_soe[e], res)
        Output._write_replan_grid_html(joinpath(gdir, "plan_cev$(e)_activity.html"), res.plan_cev_act[e], res)
    end
end
...
function write_comparison(res, outdir, name, cmp_io)
    d = res.d; nK = res.nK; labels = res.time_labels
    clock(k)    = k <= length(labels) ? labels[k] : string(k)
    cevlab(e,k) = (v = res.plan_cev_act[e][k,k]; isempty(v) ? "-" : v)
    mcslab(k)   = (v = res.plan_mcs_act[k,k];    isempty(v) ? "-" : v)
...
for c in cases
    ...
    res = run_mpc(d; time_limit_sec = 8.0, seed = 1)
...
function write_summary_html(path, df)
    ...
    println(io, "<h2>Shrinking-Horizon MPC \u2014 dummy stress cases (1 MCS, varying CEVs)</h2>")
