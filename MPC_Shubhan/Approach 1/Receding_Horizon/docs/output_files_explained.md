...
Each run is a **multi-day, cross-day receding-horizon** closed-loop MPC in **15-min
steps** over the daytime (08:00–18:00 = 40 intervals/day). Every 15-min window plans
across the rest of today **plus one or more future days**, so the plan always "sees"
tomorrow. Each night the MCS is recharged overnight (Phase 2) and parked at the grid,
while the CEV batteries **carry over** unchanged. Two batteries are tracked: the **MCS**
(mobile charging station) and each **CEV** (excavator).

> **Buffer day.** The run simulates **one extra day** beyond those requested (`n_days`)
> and **drops it** from every file/KPI below. That buffer gives the last *kept* day a
> full day of lookahead, so the "return CEVs to their start level" wrap-up lands on the
> discarded day. All outputs below therefore cover **kept days 1..n_days only**.
...
- **`day`** — which simulated day this row belongs to (1..n_days; the buffer day is dropped).
- **`gstep`** — global 15-min step index, running continuously across all kept days.
- **`k`, `clock`** — within-day interval index (1..40) and a day-tagged clock label (e.g. `D2 08:15`).
...
> **Missed work rolls over.** The daily dig/load quota is a *cumulative* soft target;
> any hours unfinished on one day are carried (and re-penalised) into the next.

## Overnight (Phase 2)

### `overnight_mcs_charge_day<N>.csv` (one per kept day)
Deterministic cheapest-hours refill that returns the MCS to its start level after each
day's 18:00:
...
## `replan_grids/day<N>/` — how the plan evolves (analyst detail)

There is **one subfolder per kept day**. Each grid shows only that day's own intervals
(the cross-day lookahead into future days is used by the optimizer but not tabulated here).
...
> (`term_tol = 0.1` kWh) the windows are feasible here, so every row is fully populated.
