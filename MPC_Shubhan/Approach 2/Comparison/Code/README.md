# Comparison driver — Approach 0 (deterministic) vs Shrinking Horizon vs Receding Horizon (both stochastic)

> This is the `Approach 2` tree's comparison driver. Approach 0 stays certainty-equivalent
> (the fixed reference); the Shrinking and Receding columns are now the **scenario-based
> stochastic MPC** controllers (see `Understanding_Stochastic_MPC.md` and each controller's
> own `docs/README.md` §13). The mechanics below — pool sharing, module aliasing, `DataLoader`
> running twice — are unchanged from the Approach 1 version of this driver; what changed is
> which model `run_mpc` solves and the `n_scenarios` option documented in the table below.

## Folder layout

This driver lives inside the `Approach 2` tree at `Comparison/Code/`:
...
**Nothing is copied.** All paths are resolved relative to `7_Comparison_main.jl`'s own
location (`@__DIR__`), so the whole `Approach 2` folder — this driver included — is
self-contained and portable; it doesn't matter where you place it. The driver calls the two
sibling codebases in place:

```
<Approach 2>/Shrinking_Horizon/code
<Approach 2>/Receding_Horizon/code
```
...
```julia
include(raw"<path to>/Approach 2/Comparison/Code/7_Comparison_main.jl")
```
...
1. Builds `Comparison/Input/` — copies the 6 confirmed-identical CSVs
   ...
   files a normal run needs, sourced from the two sibling codebases.
...
4. **Actually solves**, in this order, all from that one pool: Approach 0
   (one-shot MILP, solved once, deterministic), Approach 2a — Shrinking Horizon's
   stochastic scenario-based MPC (MILP re-solved every 15-min interval, hedging
   across `n_scenarios` sampled futures each time), Approach 2b — Receding
   Horizon's stochastic scenario-based MPC (`n_days = 1`, same hedging, every
   interval).
...
| `approach0_source` | `:shrinking` | which codebase's one-shot solver is "Approach 0" (still always the deterministic one-shot, regardless of source) |
| `n_days_receding` | `1` | days the Receding Horizon run keeps (1 = apples-to-apples with Shrinking) |
| `n_scenarios` | `5` (`ScenarioSampler.DEFAULT_N_SCENARIOS`) | scenarios sampled from the posterior at every re-solve, for **both** stochastic closed loops (Approach 2a and 2b) |
...
| `time_limit_sec`, `multi_activity`, `require_site_visit`, `single_visit_per_site`, `mcmc_samples`, `H`, `seed` | same as the sibling `run_scenario_1` mains | forwarded as-is |
...
`ScenarioSampler.jl` does **not** need this aliasing trick — unlike `ActivityPowerPool`,
nothing it returns (`sample_scenarios` gives a plain `Vector{Vector{Float64}}`, `equal_weights`
a plain `Vector{Float64}`) ever needs to cross the `ShrinkingApp`/`RecedingApp` boundary as a
shared object, so each app simply includes its own copy of `2b_ScenarioSampler.jl` (identical
file, included independently in both `module RecedingApp` and `module ShrinkingApp` blocks).
...
is a single fixed day). Both stay deterministic (certainty-equivalent) — `run_one_shot`
is unaffected by the Approach 2 stochastic extension in either codebase, so whichever
source you pick, "Approach 0" in the resulting report is always the fixed, non-hedging
reference the two stochastic columns are measured against. A 3-way comparison needs
exactly one baseline, so this driver picks ONE of the two one-shot solvers to be
"Approach 0" — the Shrinking Horizon's, by default (simpler, buffer-free). Pass
...
Comparison/Output/
  ...
  approach0_vs_shrinking_vs_receding.html    (3-way analogue of each sibling codebase's own
                                              approach0_vs_approach1.html; the "approach1"-named
                                              file/column there is now the stochastic controller)
...
Approach0-vs-Approach1 (now Approach0-vs-Approach2) timeline figure to all three
