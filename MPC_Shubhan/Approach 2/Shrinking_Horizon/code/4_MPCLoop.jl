...
function run_mpc(d, pool::ActivityPowerPool; shrinking::Bool = true, H::Int = 16,
    ...
    # so a run is still fully reproducible end to end.
    ...
    println("Running Approach 2 (STOCHASTIC scenario-based MPC, 15-min steps, $hmode horizon): $nK steps")
    ...
    for k0 in 1:nK
        ...
        # Read the solved model through the scenario-1 view: non-anticipativity
        # guarantees k0's action agrees with every scenario, and scenario 1 is
        # used as the (arbitrary but fixed) REPRESENTATIVE forward look for k > k0
        # in the replanning grids below (a diagnostic display only — it plays no
        # role in what actually gets applied or in any KPI).
        ...
        # shared (non-anticipative) k0 decision out of the stochastic model and
        # runs it through the SAME plant physics apply_and_simulate! uses for
        # Approach 0 / the deterministic controller.
        step = apply_and_simulate_stochastic!(model, k0, nK, d, pool, cursor, rng, multi_activity,
                                   soe_mcs, soe_cev, mcs_node, mcs_transit, rem_dig, rem_load, hist,
                                   real_P_ch, real_P_dch, real_L_trv, real_loc, real_P_work;
                                   plant_mode = plant, s_ref = 1)
