...
function mcs_cev_energy_profiles(model, N_c, N, M, E, T, K, delta_T, time_labels)
    ...
    for m in M
        ...
        mcs_traveling = [value(model[:L_trv_tot][m, k]/delta_T) for k in K]
...
function solve_and_analyze(
    ...
    SOE_MCS_min, tau_trv, lambda_whl_elec, lambda_CO2, rho_miss, rho_labor, eta_ch_dch, delta_T, time_labels, work_cap, p_digging, p_loading_swinging, p_traveling, scale, hours_digging, hours_loading_swinging, B,
    ...
    eta_ch_dch_vec   = isa(eta_ch_dch, Number)   ? fill(Float64(eta_ch_dch), length(M))   : collect(Float64.(eta_ch_dch))
    ...
    # Per-arc travel energy attributed to interval k (distributed across the arc's transit intervals).
    @variable(model, L_trv[M, N, N, K] >= 0)
    # Per-MCS rollup of travel energy in interval k.
    ...
    # ============================================================================
    # THIS IS THE OPTIMAL METHOD: full joint optimization of routing, charging,
    # discharging, and activity scheduling solved together as a single MILP.
    # ============================================================================
    ...
    # g_ch[m,i,k] = 1 iff MCS m actually draws grid power at grid node i during interval k.
    ...
    # At most one MCS may charge from each grid connection node per interval.
    ...
    # Travel energy per interval: total arc energy k_trv * delta_T is debited each in-transit interval.
    # (Distributed accounting: replaces the lump-sum-at-departure formulation.)
    @constraint(model, [m in M, i in N, j in N, k in K],
        L_trv[m, i, j, k] == k_trv * delta_T * y_trv[m, i, j, k]
    )

    # Total travel energy of MCS m in interval k: sum across all arcs being traversed.
    ...
    @constraint(model, [m in M, k in K],
        SOE_MCS[m, k + 1] == SOE_MCS[m, k] +
                              eta_ch_dch_vec[m] * P_ch_tot[m, k] * delta_T -
                              (P_dch_tot[m, k] * delta_T) / eta_ch_dch_vec[m] -
                              L_trv_tot[m, k]
    )
    ...
    # CEV SOE bounds at every boundary point.
