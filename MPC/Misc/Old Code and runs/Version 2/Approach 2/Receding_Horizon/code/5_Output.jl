...
function write_approach_comparison(res0, res1, out_dir)
    ...
    blurb = is_mean ?
        string("<b>Approach 0</b> was replayed under the <b>deterministic</b> plant: realized power ",
               ...
               "<b>Approach 2</b> re-plans every 15 min, hedging across sampled scenarios each step, ",
        ...
        string("Both columns are FULLY REALIZED outcomes over the ", res1.n_days_keep,
               ...
               "re-solves once per day; Approach 2 re-solves every 15 minutes). Re-run with ",
               "<code>approach0_plant = :mean</code> to see the deterministic MILP optima instead.")
    println(io, "<h2>Approach 0 (one-shot per day) vs Approach 2 (stochastic closed-loop MPC)</h2>")
    ...
    for (lbl, r) in [("Approach 0 (one-shot/day)", res0), ("Approach 2 (closed loop, stochastic)", res1)]
    ...
    for day in 1:res1.n_days_keep
        ...
        for (label, dd) in (("Approach 0", d0), ("Approach 2", d1))
