# #############################################################################
# Comparison_main.jl  —  TOP-LEVEL 3-WAY COMPARISON DRIVER
# -----------------------------------------------------------------------------
# Runs, side by side, from the SAME input data and the SAME shared power pool:
#   * Approach 0   — one-shot plan, executed open-loop, no replanning
...
#   * Approach 1b  — Receding Horizon closed-loop MPC (run with n_days = 1 so
#                    its single reported day is directly comparable to the
#                    Shrinking Horizon's single-day scope)
#
# THE CODE IS CALLED IN PLACE — NOTHING IS COPIED.
# This driver `include()`s the two source codebases directly from:
#   C:\Users\shubh\Desktop\MPC\Approach 1\Shrinking_Horizon\code
#   C:\Users\shubh\Desktop\MPC\Approach 1\Receding_Horizon\code
# Edit either codebase there and the next run picks it up automatically —
# there is no copy of either codebase living under Comparison/.
#
...
# DEFAULT PATHS
# -----------------------------------------------------------------------------
const _APPROACH1_ROOT    = raw"C:\Users\shubh\Desktop\MPC\Approach 1"
const _RECEDING_CODE_DIR  = joinpath(_APPROACH1_ROOT, "Receding_Horizon",  "code")
const _SHRINKING_CODE_DIR = joinpath(_APPROACH1_ROOT, "Shrinking_Horizon", "code")
const _RECEDING_INPUT    = joinpath(_APPROACH1_ROOT, "Receding_Horizon",  "data", "input_data")
const _SHRINKING_INPUT   = joinpath(_APPROACH1_ROOT, "Shrinking_Horizon", "data", "input_data")
const _COMPARISON_INPUT  = joinpath(_APPROACH1_ROOT, "Comparison", "Input")
const _COMPARISON_OUT    = joinpath(_APPROACH1_ROOT, "Comparison", "Output")
...
function run_comparison(; input_dir::AbstractString = _COMPARISON_INPUT,
    ...
    return _with_console_log(out_dir) do
        println("="^78)
        println("3-WAY COMPARISON — Approach 0 vs Shrinking Horizon vs Receding Horizon")
        ...
        # ---- APPROACH 1b: Receding Horizon closed-loop MPC (n_days = 1) ----
        println("\n--- Approach 1b (Receding Horizon) ---")
        ...
        apps = [
            ...
            Approach("shrinking",  "Approach 1 — Shrinking", resS, :steelblue),
            Approach("receding",   "Approach 1 — Receding",  resR, :firebrick),
        ]
