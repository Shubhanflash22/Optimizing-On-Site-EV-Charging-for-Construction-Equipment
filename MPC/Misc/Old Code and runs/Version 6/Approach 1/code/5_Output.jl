...
export write_outputs, write_detailed_output
...
function csv_mcs_cev_soe(res)
    ...
    for m in d.M
        ...
        csv[!, "MCS_$(m)_Traveling_kW"]   = zeros(length(K))  # transit does not draw from the battery
...
# capping). Applies identically whether res came from run_one_shot or run_mpc,
# for any of the five approaches.
...
# res.replan_by_day[day], one full set of DAY-LOCAL grids per day. Writes one
# grid folder per KEPT day.
...
# =============================================================================
# DETAILED OUTPUT (opt-in): the full per-15-min plan/realized CSVs, for both
# the CEV(s) (DetailedPlanLog/RealizedTupleLog) and the MCS
# (MCSPlanLog/MCSRealizedLog) -- see run_mpc's `detailed_output` kwarg. A
# no-op (writes nothing) if `res` was produced with detailed_output = false,
# since res.detailed_plan_df etc are `nothing` in that case.
# =============================================================================
function write_detailed_output(res, out_dir::AbstractString)
    mkpath(out_dir)
    if res.detailed_plan_df !== nothing
        CSV.write(joinpath(out_dir, "plan_full.csv"), res.detailed_plan_df)
    end
    if res.detailed_realized_df !== nothing
        CSV.write(joinpath(out_dir, "realized_tuple.csv"), res.detailed_realized_df)
    end
    if res.detailed_mcs_plan_df !== nothing
        CSV.write(joinpath(out_dir, "MCS_plan_full.csv"), res.detailed_mcs_plan_df)
    end
    if res.detailed_mcs_realized_df !== nothing
        CSV.write(joinpath(out_dir, "MCS_realized_tuple.csv"), res.detailed_mcs_realized_df)
    end
    return out_dir
end
