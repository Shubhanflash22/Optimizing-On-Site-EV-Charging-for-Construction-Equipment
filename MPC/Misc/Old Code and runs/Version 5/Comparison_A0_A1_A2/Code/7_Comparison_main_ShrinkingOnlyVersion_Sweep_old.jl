...
# -----------------------------------------------------------------------------
# Writes the 4 CSVs (A1_plan_full, A1_realized_tuple, A2_plan_full,
# A2_realized_tuple) for ONE mode into a COMPLETELY SEPARATE tree:
...
function _write_detailed_output(res, approach_prefix::AbstractString, out_dir::AbstractString, mode)
    detailed_root = replace(out_dir, "Output_AllRuns" => "Detailed_Output_AllRuns")
...
function run_comparison_sweep(; input_dir::AbstractString = _COMPARISON_INPUT,
    ...
    return _with_console_log(out_dir) do
        ...
        for mode in modes
            ...
            # ---- DETAILED OUTPUT (opt-in): separate tree, A1S + A2S only ----
            if detailed_output
                d1 = _write_detailed_output(resA1S, "A1", out_dir, mode)
                d2 = _write_detailed_output(resA2S, "A2", out_dir, mode)
                println("  detailed output ($(mode)) -> $(d1)  (A1_*.csv, A2_*.csv)")
            end
