...
function run_comparison(; input_dir::AbstractString = _COMPARISON_INPUT,
                          ...
                          combos = _ALL_COMBOS,
                          seed::Int = 1,
                          # NEW: override the starting CEV battery charge for this run.
                          # `nothing` (default) = use whatever's in ev_data.csv, unchanged
                          # -- so every existing call site behaves exactly as before.
                          # Pass a number (e.g. 6.91) to run this whole comparison at that
                          # starting charge instead, without touching ev_data.csv itself.
                          # Applied identically to all four loaded datasets (dA1S/dA2S/
                          # dA1R/dA2R) right after they're loaded, below.
                          soe_cev_ini_override::Union{Nothing, Real} = nothing)
    ...
    return _with_console_log(out_dir) do
        ...
        # Apply the starting-charge override, if given -- SOE_CEV_ini is a
        # Vector{Float64} (one entry per CEV; this dataset has 1 CEV), read
        # straight from ev_data.csv at load time. NamedTuples are immutable,
        # so rebuild each with just that one field replaced via merge; every
        # other field (SOE_CEV_min/max, MCS state, prices, quotas, ...) is
        # untouched. Applied identically to all four so A0's own terminal
        # target (which reads d.SOE_CEV_ini too) moves together with it.
        if soe_cev_ini_override !== nothing
            new_ini = fill(Float64(soe_cev_ini_override), length(dA1S.SOE_CEV_ini))
            dA1S = merge(dA1S, (; SOE_CEV_ini = new_ini))
            dA1R = merge(dA1R, (; SOE_CEV_ini = new_ini))
            dA2S = merge(dA2S, (; SOE_CEV_ini = new_ini))
            dA2R = merge(dA2R, (; SOE_CEV_ini = new_ini))
        end
