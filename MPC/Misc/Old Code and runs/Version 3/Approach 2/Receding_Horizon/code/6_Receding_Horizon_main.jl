# #############################################################################
# Receding_Horizon_main.jl  —  TOP-LEVEL DRIVER (thin)   [APPROACH 2: STOCHASTIC MPC]
# -----------------------------------------------------------------------------
...
# length, whatever time of day it is): (1) SAMPLES a small set of scenarios from a
# power model that is fitted ONCE and held FIXED, (2) OPTIMISES a single MILP that
# hedges across all of them at once (non-anticipativity: one shared action for
# right now, feasible in every scenario), and (3) APPLIES only that shared first
# interval to the stochastic plant (realized power is drawn from the fixed
# posterior; the model is never re-fitted online). See
# docs/Understanding_Stochastic_MPC.md for the full explanation. Every window's
# own 24h-out terminal point requires the MCS/CEV
...
#                          each re-solve consumes.
#   3_MCSModel.jl          the fixed-length 24h rolling window MILP: `build_window_model`
#                          (deterministic, Approach 0 baseline) AND
#                          `build_window_model_stochastic` (scenario-linked, this
#                          controller). Overnight recharge is part of the window's
#                          own terminal target either way.
#   4_MPCLoop.jl           the multi-day closed loop (sample + optimise + apply)
...
function run_scenario_1(; mode::Symbol = :synthetic,
                          ...
                          # APPROACH 2: number of scenarios sampled from the posterior at
                          # every re-solve (see 2b_ScenarioSampler.jl). 5 by default.
    ...
    return _with_console_log(out_dir) do
        ...
        res = run_mpc(d, pool; plant = :sampled, time_limit_sec = time_limit_sec, multi_activity = multi_activity,
                         ...
                         mcmc_samples = mcmc_samples, n_scenarios = n_scenarios,
                         n_days = n_days, seed = seed)
        ...
        println("           approach0_vs_approach1.html  (Approach 0 one-shot/day vs Approach 2 stochastic closed-loop, totals + per-day;")
...
function _print_approach_summary(res0, res1)
    ...
    gap_lbl = res0.plant === :mean ?
        "  A2 - A0 (drift + hedging/re-planning, net)  " :
        "  A2 - A0 (value of hedging + intra-day re-planning)"
    ...
    @printf("A2 stochastic closed loop (%d scenarios) : \$%.2f  (%.2f h transit, %.2f h missed)\n",
            res1.n_scenarios, tot(res1), res1.transit_intervals * res1.d.delta_T, res1.missed)
    @printf("%s: %+.2f  (negative = A2 cheaper)\n", gap_lbl, tot(res1) - tot(res0))
    res0.plant === :mean && println("  NOTE: A0 is DETERMINISTIC here, so this gap mixes plan drift with the",
                                    "\n        value of hedging + re-planning. Re-run with approach0_plant = :sampled",
                                    "\n        to separate the two.")
