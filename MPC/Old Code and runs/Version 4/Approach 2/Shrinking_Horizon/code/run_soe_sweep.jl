# #############################################################################
# run_soe_sweep.jl  —  Initial-SOE sensitivity sweep for the :input case. [APPROACH 2]
# -----------------------------------------------------------------------------
...
# horizon STOCHASTIC (scenario-based) closed loop (each window solved to the MIP gap, NO time limit) for
...
#   approach0_vs_approach1.html     (Approach 0 one-shot vs Approach 2 stochastic closed-loop)
...
# APPROACH 2: scenarios sampled from the posterior at every re-solve. Overridable
# by the master runner via MASTER_N_SCENARIOS, exactly like MASTER_A0_PLANT below.
const N_SCENARIOS = (@isdefined(MASTER_N_SCENARIOS) && MASTER_N_SCENARIOS isa Int) ?
                    MASTER_N_SCENARIOS : DEFAULT_N_SCENARIOS
...
for (idx, soe) in enumerate(soe_vals)
    ...
    row = _with_console_log(dst) do
        ...
        # ---- APPROACH 2: scenario-based stochastic closed-loop MPC ----
        res = run_mpc(d, pool; time_limit_sec = Inf, plant = :sampled, n_scenarios = N_SCENARIOS, seed = 1)
...
println(io, "<table><tr>",
    ...
    "<th>A0 (\$)</th><th>&Delta; A2&minus;A0<br>(\$)</th><th>SOE<br>clamps</th>",
...
println(io, "<p class=\"muted\" style=\"margin-top:20px\"><b>Column notes.</b> ",
            ...
            "<b>&Delta; A2&minus;A0</b> = Realised &minus; A0 (negative = the closed loop was cheaper). ",
