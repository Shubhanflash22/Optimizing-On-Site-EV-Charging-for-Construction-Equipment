...
# one interval at a time (SHRINKING horizon). See
# docs/Understanding_Stochastic_MPC.md for the full explanation and a worked
# numerical example; Approach 1 (certainty-equivalent MPC) is the deterministic
# sibling this extends — see that project's own README for how the two compare.
#
...
#   3_MCSModel.jl          the window MILP: `build_window_model` (deterministic,
#                          Eq. 1-13, used for the Approach 0 baseline) AND
#                          `build_window_model_stochastic` (the scenario-linked
#                          version this controller solves every 15 min)
...
function run_scenario_1(; mode::Symbol = :synthetic,
    ...
    return _with_console_log(out_dir) do
        ...
        # frozen d.prior_mu/d.prior_sigma, so Approach 0 and Approach 2 draw
        ...
        #                no feedback (what re-planning/hedging is being credited against).
        ...
        println("           approach0_vs_approach1.html  (Approach 0 one-shot vs Approach 2 closed-loop, both fully")
...
# under the stochastic plant with no feedback, and the scenario-based closed loop
# hedging against it. Splits the headline A2-vs-A0 gap into "cost of drift" and
# "value of scenario-based re-planning (hedging + re-solving)".
function _print_approach_summary(res0, res1)
    ...
    gap_lbl = res0.plant === :mean ?
        ...
        "  A2 - A0 (value of hedging + re-planning)    "
