# #############################################################################
#  RUN_ALL.jl  —  ONE-CLICK MASTER RUNNER
#  Place at:  C:\Users\shubh\Desktop\MPC\Approach 1\RUN_ALL.jl
#  Run with:  julia "C:\Users\shubh\Desktop\MPC\Approach 1\RUN_ALL.jl"
#             (or just include() it from the Julia REPL)
# -----------------------------------------------------------------------------
#  Runs, in this order:
#     1. Shrinking Horizon  — :input
#     2. Shrinking Horizon  — :synthetic
#     3. Shrinking Horizon  — SOE sweep
#     4. Receding  Horizon  — :input
#     5. Receding  Horizon  — :synthetic
#     6. Receding  Horizon  — SOE sweep
#     7. Comparison         — 3-way (Approach 0 vs Shrinking vs Receding)
#
...
# ║                feedback to correct it. Approach 1 faces the SAME draws,   ║
...
# ║                Approach 0's KPIs ARE the MILP's own optimum. Approach 1   ║
# ║                is still stochastic, so the reported gap then MIXES plan   ║
# ║                drift with the value of re-planning.                       ║
...
const ROOT            = raw"C:\Users\shubh\Desktop\MPC\Approach 1"
...
function _stage_source(id::Symbol)
    ...
    elseif id === :shrinking_sweep
        ...
        # MASTER_A0_PLANT must be defined BEFORE the include so the sweep's
        # `const A0_PLANT` picks it up instead of its own default.
        return """
...
function run_all()
    ...
    println("#  PLANT MODE : :", PLANT_MODE,
            PLANT_MODE === :mean ?
              ...
              "   (Approach 0 stochastic — gap isolates re-planning value)")
