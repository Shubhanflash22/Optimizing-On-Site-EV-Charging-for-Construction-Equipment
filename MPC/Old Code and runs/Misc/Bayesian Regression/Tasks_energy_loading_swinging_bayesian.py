...
# BATCH / STEP-0 MODE ---------------------------------------------------------
# When this script is invoked with a destination path argument (e.g. from the
# Julia step-0 driver: `python this.py <parameters.csv>`), run HEADLESS so no
# blocking matplotlib windows stall the pipeline, and export the fitted powers
# to that CSV at the end. Setting MPLBACKEND before matplotlib is imported forces
# the non-interactive Agg backend.
_BATCH_OUT = sys.argv[1] if len(sys.argv) > 1 else None
if _BATCH_OUT:
...
# Make the hardcoded Excel paths portable: redirect any pd.read_excel(path) to a
# file of the same basename sitting NEXT TO this script, if one exists. This lets
# the same script run on the original macOS layout AND on a Windows checkout where
# the .xlsx files live in the script's own folder, without editing 23 read calls.
_HERE = os.path.dirname(os.path.abspath(__file__))
_orig_read_excel = pd.read_excel
def _read_excel_anywhere(io, *args, **kwargs):
    try:
        local = os.path.join(_HERE, os.path.basename(str(io)))
    except Exception:
        local = None
    return _orig_read_excel(local if (local and os.path.exists(local)) else io, *args, **kwargs)
pd.read_excel = _read_excel_anywhere
...
"""""""""""""""""""""""""""""""""""""""EXPORT TO MPC parameters.csv (STEP 0)"""""""""""""""""""""""""""""""""

def _export_to_mpc_parameters(trace_export, dest_csv):
    """Write the full-data posterior MEAN and per-activity SD for
    Digging / Loading+Swinging / Traveling into the MPC parameters.csv, updating
    existing rows in place and appending any that are missing. The per-activity SD
    is the posterior standard deviation of each activity power x (its estimation
    uncertainty), which the Julia DataLoader reads as the per-activity prior_sigma.
    grading == 'False' column order: [Digging, Loading+Swinging, Traveling, Idling, Mixing].
    """
    xm = trace_export.posterior["x"].mean(dim=("chain", "draw")).values
    xs = trace_export.posterior["x"].std(dim=("chain", "draw")).values
    rows = {
        "p_digging":              (round(float(xm[0]), 4), "kW", "Power of digging (Bayesian posterior mean)"),
        "p_loading_swinging":     (round(float(xm[1]), 4), "kW", "Power of loading+swinging (Bayesian posterior mean)"),
        "p_traveling":            (round(float(xm[2]), 4), "kW", "Power of traveling (Bayesian posterior mean)"),
        "sigma_digging":          (round(float(xs[0]), 4), "kW", "Per-activity std of digging power (posterior SD)"),
        "sigma_loading_swinging": (round(float(xs[1]), 4), "kW", "Per-activity std of loading+swinging power (posterior SD)"),
        "sigma_traveling":        (round(float(xs[2]), 4), "kW", "Per-activity std of traveling power (posterior SD)"),
    }
    dest_dir = os.path.dirname(dest_csv)
    if dest_dir and not os.path.isdir(dest_dir):
        print(f"[step0] destination folder not found, skipping export: {dest_csv}")
        return
    if os.path.exists(dest_csv):
        pdf = pd.read_csv(dest_csv)
    else:
        pdf = pd.DataFrame(columns=["Parameter", "Value", "Unit", "Description"])
    for col in ("Parameter", "Value", "Unit", "Description"):
        if col not in pdf.columns:
            pdf[col] = ""
    pdf["Value"] = pdf["Value"].astype(object)
    for name, (val, unit, desc) in rows.items():
        mask = pdf["Parameter"].astype(str).str.strip() == name
        if mask.any():
            pdf.loc[mask, "Value"] = val
        else:
            pdf = pd.concat(
                [pdf, pd.DataFrame([{"Parameter": name, "Value": val, "Unit": unit, "Description": desc}])],
                ignore_index=True,
            )
    pdf.to_csv(dest_csv, index=False)
    print(f"[step0] wrote fitted powers + per-activity SD to {dest_csv}")
    print("[step0]   means :", {k: v[0] for k, v in rows.items() if k.startswith("p_")})
    print("[step0]   sigmas:", {k: v[0] for k, v in rows.items() if k.startswith("sigma_")})

if _BATCH_OUT:
    _export_to_mpc_parameters(trace_full, os.path.abspath(_BATCH_OUT))
...
if (not _BATCH_OUT) and sys.flags.interactive == 0 and plt.get_fignums():
    plt.ioff()
    plt.show()