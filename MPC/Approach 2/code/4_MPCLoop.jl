...
# -----------------------------------------------------------------------------
# See the identical docstring in Approach 1/code/4_MPCLoop.jl
...
function run_mpc(d, pool::ActivityPowerPool; shrinking::Bool = true, H::Int = 16,
    ...
    # day and saved into replan_by_day[day] at that day's end -- see the
    # identical note in Approach 1's run_mpc.
    ...
    for day in 1:n_day_run
        ...
        for k0 in 1:nKd                      # k0 is DAY-LOCAL (1..nKd)
            ...
            # K_win stays DAY-LOCAL -- see the identical note in Approach 1's
            # run_mpc: this is what makes it a shrinking
