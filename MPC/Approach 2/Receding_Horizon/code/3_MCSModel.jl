...
function build_window_model(d, K_win, soe_mcs0, soe_cev0, mcs_node0, mcs_transit0,
    ...
    # This penalty applies only to the deterministic objective here; the
    # scenario-based objective below already hedges timing risk via scenarios.
    Kvec = collect(K)
    ...
    # ---- activity scheduling ----
    # Eq. 8b, restricted to is_working (wd(k): is_working is a single-day profile
    # repeated across the multi-day window).
...
function build_window_model_stochastic(d, K_win, soe_mcs0, soe_cev0, mcs_node0, mcs_transit0,
    ...
    # CEV SOE recursion: previous + energy received from the MCS (scaled by the CEV's
    # own charge-acceptance efficiency, eta_ch_dch_cev) - energy spent working.
    ...
    @constraint(model, [i in N_c, e in E, k in K, s in S_scen; d.is_working[i, e, wd(k)]],
        sum(u[e, i, a, k, s] for a in B) == d.A[i, e])
    @constraint(model, [i in N_c, e in E, a in B, k in K, s in S_scen; !d.is_working[i, e, wd(k)]],
        u[e, i, a, k, s] == 0)
    @constraint(model, [i in N_c, e in E, a in B, k in K, s in S_scen], u[e, i, a, k, s] <= d.A[i, e])
    @constraint(model, [i in N_c, e in E, k in K, s in S_scen],
        mu[i, e, k, s] <= (d.is_working[i, e, wd(k)] ? u[e, i, B[4], k, s] : 1))
    @constraint(model, [i in N_c, e in E, k in K, s in S_scen],
