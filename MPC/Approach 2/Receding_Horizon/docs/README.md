...
* **Work shift (synthetic):** productive work is available inside the daytime block only;
  outside it `is_working` is `false` for that (node, CEV, interval) and every activity is forced off (Eq. 8b, §8.9) -- including idle. (In `:input` mode the availability comes from
  `work_flexible.csv`.)
...
These caps become the `is_working` mask (any value > 0 = working hours, §8.9) and, via the last non-zero column, infer `day_end_hour`. Sits alongside `live_powers.csv` (LIVE_DATA_MODE) -- an unrelated, optional file of real recorded per-activity powers for the plant simulation, not the MILP's work-availability input.
