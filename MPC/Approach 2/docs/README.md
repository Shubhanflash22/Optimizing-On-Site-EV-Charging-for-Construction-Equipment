...
plant pool as one of four fixed scenarios by biasing the z-score used in `mu[a] + sd[a] * z`,
without touching `n_samples`, cursors, or `next_power!`: `:near_mean` clusters draws within
**~0.5σ** of the mean; `:high` and `:low` push every draw **≥2σ** above/below the mean
respectively; and `:spread_wide` pushes every draw **≥2σ** from the mean with a random sign. Each
mode still adds a small amount of randomness on top of its anchor, so repeated draws in the same
mode differ. This only affects the **plant** pool (Fork B above); the scenario set the Approach 2
controller plans against (`sample_scenarios`, §13) is unrelated, drawn fresh every re-solve, and
unaffected by this `mode`. This is for running the same scenario again under a deliberately
biased plant (e.g. to see how Approach 0/Approach 2's KPIs move if the machines consistently draw
high/low/near/scattered power) — it does not change the calibrated `(μ, σ)` themselves.
...
* **Change 5 — `n_day_run` (currently 1), internal day loop.** `run_mpc`/`run_one_shot` in
  ...
  instead of being silently overwritten, and every
