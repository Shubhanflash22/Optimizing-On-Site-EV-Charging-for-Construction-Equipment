...
* **Work shift (synthetic):** `work_start_hour = 8`, `work_end_hour = 17`, with a
  ...
  outside that, `is_working` is `false` for that (node, CEV, interval) and every activity
  is forced off (Eq. 8b, §8.9) -- including idle. (In `:input` mode the availability comes
  from `work_flexible.csv`; any nonzero value in a cell means "working" there.)
...
**Draw mode (sensitivity sweep).** `draw_activity_power_pool` also takes an optional `mode`
keyword (default `:normal`, the unbiased behaviour above, unchanged) that regenerates the whole
pool as one of four fixed scenarios by biasing the z-score used in `mu[a] + sd[a] * z`, without
touching `n_samples`, cursors, or `next_power!`: `:near_mean` clusters draws within **~0.5σ** of
the mean; `:high` and `:low` push every draw **≥2σ** above/below the mean respectively; and
`:spread_wide` pushes every draw **≥2σ** from the mean with a random sign. Each mode still adds a
small amount of randomness on top of its anchor, so repeated draws in the same mode differ. This
is for running the same scenario again under a deliberately biased plant (e.g. to see how
Approach 0/Approach 1's KPIs move if the machines consistently draw high/low/near/scattered power)
— it does not change the calibrated `(μ, σ)` themselves.
...
These caps become the `is_working` mask over the full 24 h horizon: a value greater than 0 marks that (node, CEV, interval) as working hours (§8.9). Sits alongside `live_powers.csv` (§3.x, LIVE_DATA_MODE) -- an unrelated, optional file of real recorded per-activity powers for the plant simulation, not the MILP's work-availability input.
...
Every rule is **HARD** unless marked **SOFT**.
...
* Exactly one activity applies only during working hours; outside them, EVERY activity (including idle) is forced off: `Σ_a u = A` when `is_working`, else `u = 0` for all `a` (Eq. 8b).
...
> the equality rather than written as a separate explicit rule.
...
* **Change 3 — terminal SOE_CEV shortfall penalty (Issue 1).** New `_terminal_soe_shortfall` in
  ...
  already feeds `rem_dig`/`rem_load`/`missed`. Converted to hours using the day's *required*
  weighted work-power rate (the full dig/load quota, not what was actually achieved, so a bad
  day's own shortfall can't inflate the hours-conversion it's measured against — and not a flat
  average either), charged directly onto TOTAL cost as a new `shortfall_cost` line. Applies
  uniformly to all five approaches — see `5_Output.jl` and the three comparison harnesses for the
  downstream reporting changes.
* **Change 5 — `n_day_run` (currently 1), internal day loop.** `run_mpc` in `4_MPCLoop.jl` now
  contains its OWN internal day loop (an external `run_multiday` wrapper was tried first, then
  retired once the day loop was moved inside). Within each day, the window is DAY-LOCAL — it shrinks
  ...
  **Follow-up fix — `replan_by_day`:** the plan grids (`plan_grid_kW`/`plan_mcs_soe`/
  ...
  builds a fresh set of grids every day and saves each into `replan_by_day[day]`, and every
  `5_Output.jl` function that reads them was updated to use `res.replan_by_day[1]` (day 1's plan,
  matching the report's own
