...
function run_mpc(d, pool::ActivityPowerPool; shrinking::Bool = true, H::Int = 16,
    ...
    # the WHOLE run, day boundaries included (CHANGE 5 -- only the work QUOTA
    # below is a once-per-calendar-day event; hist itself never resets here).
    ...
    # and saved into replan_by_day[day] at that day's end -- NOT a single flat
    # grid that gets silently overwritten day after day. For n_day_run = 1
    ...
    for day in 1:n_day_run
        ...
        for k0 in 1:nKd                      # k0 is DAY-LOCAL (1..nKd)
            ...
            for k in K_win
                ...
                # DETAILED OUTPUT -- MCS: the same full remaining-horizon plan
                # capture as the CEV rows above, just for the MCS's own
                # status/node/charge/discharge/SOE.
                if detailed_output
                    for m in d.M
                        log_mcs_plan_row!(mcs_plan_log, d; day, resolve_step = k0, offset_step = k,
                                          mcs = m, mcs_status_planned = plan_mcs_act[k0, k],
            ...
            if detailed_output
                ...
                # for THIS executed interval, straight from the real_* arrays
                # apply_and_simulate! just filled in above (nothing new derived).
                for m in d.M
                    log_mcs_realized_row!(mcs_realized_log, d; day, step = k0, mcs = m,
