    ...
    for e in d.E
        ...
        if out_idx !== nothing
            if site !== nothing
                ...
                # the FULL activity set B, not just the 3 productive ones). idle
                # currently draws 0 kW so row[4]*pt[4] contributes nothing today, but
                # excluding it would silently under-report realized work power the
                # moment idle power (p_idling) is ever made nonzero -- this line must
                # stay in sync with `work_kW` below, which sums over all 4 via dot().
...
function _terminal_soe_shortfall(d, soe_cev_end, rem_dig, rem_load)
    ...
    # Uses the full REQUIRED quota, not what was actually achieved (rem_dig/
    # rem_load), so the hours-conversion rate stays fixed regardless of how
    # the day itself went.
    required_dig_h  = sum(d.hours_digging)
    required_load_h = sum(d.hours_loading_swinging)
...
function run_mpc(d, pool::ActivityPowerPool; plant::Symbol = :sampled,
    ...
    println("Running Approach 1 (closed-loop MPC, 15-min steps, multi-day receding horizon): $n_kept steps ($n_days_keep kept days)")
    ...
    for day in 1:D_total
        ...
        # (see initialization above the day loop).
        ...
        for k0 in 1:nKd
            ...
            for k in K_win
                ...
                for e in d.E
                    ...
                    site = findfirst(i -> d.A[i, e] == 1, d.N)
                    site !== nothing && (plan_cev_act[e][k0, kl] = activity_label(model, d, e, site, k))
                end
                plan_mcs_act[k0, kl] = mcs_status_label(model, d, k)
            end
