...
module A1ShrinkingApp
    ...
    # TEMP-RUN PATCH: hardcoded for the same reason as A1RecedingApp above.
    ...
    return _with_console_log(out_dir) do
        ...
        pool = if mode == :live_data
            A1RecedingApp.Common.draw_activity_power_pool_live(
                dA1S.E, A1RecedingApp.DataLoader.load_live_powers(input_dir);
                rng = MersenneTwister(seed))
        else
            # mode = :normal -> unbiased draws (unchanged); see 1_Common.jl's
            # "DRAW MODE" doc for the 4 sensitivity-sweep modes (see
            # 7_Comparison_main_RecedingOnlyVersion_Sweep.jl for a driver that
            # sweeps all 4 for the Receding-only comparison).
            A1RecedingApp.Common.draw_activity_power_pool(dA1S.E, dA1S.prior_mu, dA1S.prior_sigma;
                                                           n_samples = n_samples,
                                                           rng = MersenneTwister(seed),
                                                           mode = mode)
        end
