# #############################################################################
# 7_Comparison_main_RecedingOnlyVersion_Sweep.jl  —  DRAW-MODE SENSITIVITY SWEEP
# -----------------------------------------------------------------------------
# Runs the SAME 3-way comparison as 7_Comparison_main_RecedingOnlyVersion.jl
# (A0 one-shot vs A1-Receding vs A2-Receding, from the same shared power pool)
# FIVE TIMES OVER — once per plant `mode` supported by `draw_activity_power_pool`
# in 1_Common.jl (see that function's "CHANGE 6" header for the full detail):
#
...
# differs) and its own output subfolder, so nothing from one mode's run can
# leak into another's:
#
#   Output/normal/A0_A1R/           Output/near_mean/A0_A1R/        … etc
#   Output/normal/A0_A2R/           Output/near_mean/A0_A2R/
#   Output/normal/A0_A1R_A2R/       Output/near_mean/A0_A1R_A2R/
#   Output/normal/A1R_A2R/          Output/near_mean/A1R_A2R/
...
#
# FIXED AT n_days_receding = 1 (1-day runs) FOR ALL FOUR MODES, per request —
# override via the `n_days_receding` keyword to `run_comparison_mode_sweep`
# if a longer horizon is ever needed; nothing else about the sweep depends on
# that value.
#
...
# `include()`s 7_Comparison_main_RecedingOnlyVersion.jl (with its own
# auto-run disabled) to get the SAME `A1RecedingApp` / `A2RecedingApp`
...
# comparison — this file supplies its own entry point (`run_comparison_mode_sweep`)
# at the bottom instead.
const COMPARISON_NO_AUTORUN = true
include(joinpath(@__DIR__, "7_Comparison_main_RecedingOnlyVersion.jl"))

using Printf
using Random

# The 5 draw modes swept by default (see header above). Pass a different
# `modes` tuple/vector to `run_comparison_mode_sweep` to run a subset, e.g.
# during a quick test.
...
# `n_days_receding` defaults to 1 (1-day runs) here, unlike the base driver's
# multi-day default, since this sweep is meant for fast, repeated 1-day checks
# across all five plant modes.
# =============================================================================
function run_comparison_mode_sweep(; input_dir::AbstractString = _COMPARISON_INPUT,
                                     ...
                                     n_days_receding::Int = 1,                  # 1-day runs, per request
    ...
    approach0_source in (:a1_receding, :a2_receding) ||
        error("run_comparison_mode_sweep: approach0_source must be :a1_receding or :a2_receding")
    approach0_plant in (:sampled, :mean) ||
        error("run_comparison_mode_sweep: approach0_plant must be :sampled or :mean")
    isempty(modes) && error("run_comparison_mode_sweep: `modes` must list at least one mode")
    ...
    return _with_console_log(out_dir) do
        ...
        println("n_days_receding   : $(n_days_receding)  (1-day runs)")
        ...
        # Same sizing rule as the base driver: cover the buffer-inclusive
        # length so next_power! never runs out mid-run (see that driver's
        # header note on pool sizing for the full rationale).
        ...
        for mode in modes
            ...
            # literal SAME object into all three runs for THAT mode (same
            # cross-codebase aliasing trick as the base driver — see its
            # header for why one ActivityPowerPool object can be handed to
            # both A1RecedingApp and A2RecedingApp). ----
            pool = _timed_status("building :$(mode) power pool ($(n_samples) samples/entity-activity)") do
                if mode == :live_data
                    A1RecedingApp.Common.draw_activity_power_pool_live(
                        dA1R.E, A1RecedingApp.DataLoader.load_live_powers(input_dir);
            ...
            resA1R = _timed_status("[:$(mode)] Approach 1 - Receding solve (n_days = $(n_days_receding))") do
            ...
            resA2R = _timed_status("[:$(mode)] Approach 2 - Receding solve ($(n_scenarios) scenarios, n_days = $(n_days_receding))") do
            ...
            # under Output/<mode>/ -- same 3 solved results, sliced into as
            # many output folders as `combos` lists, exactly like the base
            # driver's single-mode run ----
            ...
            push!(mode_summaries, (; mode, res0, resA1R, resA2R))
        end
        ...
        for row in mode_summaries
            ...
            @printf("%-28s %14.2f %14.2f %14.2f\n", "Grid energy (kWh)",        row.res0.total_energy,   row.resA1R.total_energy,   row.resA2R.total_energy)
            @printf("%-28s %14.2f %14.2f %14.2f\n", "Energy cost (USD)",        row.res0.total_cost,     row.resA1R.total_cost,     row.resA2R.total_cost)
            @printf("%-28s %14.2f %14.2f %14.2f\n", "CO2 (kg)",                 row.res0.total_co2,      row.resA1R.total_co2,      row.resA2R.total_co2)
            @printf("%-28s %14.2f %14.2f %14.2f\n", "NCD peak (kW)",            row.res0.nc_peak,        row.resA1R.nc_peak,        row.resA2R.nc_peak)
            @printf("%-28s %14.2f %14.2f %14.2f\n", "OPD peak (kW)",            row.res0.op_peak,        row.resA1R.op_peak,        row.resA2R.op_peak)
            @printf("%-28s %14.2f %14.2f %14.2f\n", "Missed work (h)",          row.res0.missed,         row.resA1R.missed,         row.resA2R.missed)
            @printf("%-28s %14.3f %14.3f %14.3f\n", "Terminal shortfall (kWh)", row.res0.shortfall_kWh,  row.resA1R.shortfall_kWh,  row.resA2R.shortfall_kWh)
        end
        println("="^78)
        ...
        open(summary_csv, "w") do io
            ...
            for row in mode_summaries
                for (label, res) in (("A0", row.res0), ("A1R", row.resA1R), ("A2R", row.resA2R))
