...
UPDATE: Paths are now passed as CLI arguments so this script can be called
        in a loop for multiple models without editing the file each time.
"""
...
# CLI Arguments
# ============================
parser = argparse.ArgumentParser(description="Part 11 — Grid Search Optimization & Evaluation")
parser.add_argument("--model_path",  required=True,  help="Path to resnet3d_best_kinetics_2.pth")
parser.add_argument("--mean_path",   required=True,  help="Path to dataset_mean.npy")
parser.add_argument("--std_path",    required=True,  help="Path to dataset_std.npy")
parser.add_argument("--video_path",  required=True,  help="Path to input video (Day_3.mp4)")
parser.add_argument("--tasks_xlsx",  required=True,  help="Path to Tasks.xlsx ground truth file")
parser.add_argument("--output_dir",  required=True,  help="Directory where optimization results are saved")
parser.add_argument("--yolo_path",   default="/data_avik/Ten_new_days/best.pt",
                    help="Path to best.pt (default: /data_avik/Ten_new_days/best.pt)")
...
TASKS_XLSX      = Path(args.tasks_xlsx)
...
def evaluate_run(predictions_idx, confidences, activity_names, fps, gt_data,
                 save_plots=False, output_dir=None, run_name=None):
    """
    Compares predictions against ground truth and returns accuracy.
    During the grid search (save_plots=False) no files or folders are created.
    When save_plots=True (top-5 post-processing) confusion matrix + timeline
    plots are saved into output_dir.
    """
    # Convert list of indices to dict keyed by frame index
    pred_dicts = {}
    ...
    for sheet_name, raw_segs in gt_data.items():
        ...
        if raw_segs[0][0] > max_pred_time: continue   # sheet doesn't overlap video

        gt_map = {}
        for s, e, lab in raw_segs:
            e = min(e, max_pred_time)
            if s >= e: continue
            for f in range(int(round(s * fps)), int(round(e * fps))):
                gt_map[f] = lab
        ...
        for f in sorted(gt_map.keys()):
            if f in pred_dicts:
                ...
                sheet_comparison.append((f, gt_lab, pred_lab, gt_lab == pred_lab))

        if not sheet_comparison: continue
        all_comparison.extend(sheet_comparison)

        # Plots only for top-5 post-run
        if save_plots and output_dir and run_name:
            output_dir.mkdir(exist_ok=True, parents=True)
            plot_timeline(raw_segs, pred_dicts, fps,
                          output_dir / f"{run_name}_{sheet_name}_timeline.png",
                          f"{run_name} - {sheet_name}")

    if not all_comparison:
        print("    Warning: No overlapping ground truth found for evaluation.")
        return 0.0

    correct  = sum(1 for *_, m in all_comparison if m)
    accuracy = correct / len(all_comparison)

    # Confusion matrix only for top-5 post-run
    if save_plots and output_dir and run_name:
...
def main():
    ...
    for i, values in enumerate(param_combinations):
        ...
        # Progress print every 100 runs (not per-run to keep logs clean)
        if (i + 1) % 100 == 0 or i == 0:
            print(f"  [{i+1}/{total_combos}] {run_id} running...")
        ...
        # C. Evaluate (no plots, no folder, no CSV — just returns accuracy float)
        acc = evaluate_run(final_preds, confs, activity_names, target_fps, gt_data)

        # D. Log to Master Report only
    ...
    print(f"\n[Step 4] Grid search complete. {total_combos} combinations evaluated.")

    # ── TOP-5: generate confusion matrix + timeline plots ──────────────────
    print("\n[Step 5] Identifying Top 5 runs by accuracy and generating plots...")

    report_df = pd.read_csv(master_report_path)
    report_df['accuracy_percent'] = pd.to_numeric(report_df['accuracy_percent'], errors='coerce')
    top5 = report_df.nlargest(5, 'accuracy_percent')

    print(f"\n{'Rank':<6} {'Run':<12} {'Accuracy':>10} {'Prod (LCY/hr)':>14}")
    print("-" * 46)

    for rank, (_, row) in enumerate(top5.iterrows(), 1):
        run_id = row['run_folder']
        params = {k: row[k] for k in keys}
        print(f"  #{rank:<4} {run_id:<12} {float(row['accuracy_percent']):>9.2f}%  {float(row['productivity_lcy_hr']):>13.2f}")

        # Re-run post-processing for this combo
        final_preds = apply_post_processing(raw_preds, confs, bboxes, activity_names, params, target_fps)

        # Build confusion matrix directly and save flat into OUTPUT_BASE_DIR
        cm      = np.zeros((len(ALL_CLASSES), len(ALL_CLASSES)), dtype=int)
        idx_map = {c: i for i, c in enumerate(ALL_CLASSES)}
        max_pred_time = len(final_preds) / target_fps

        for sheet_name, raw_segs in gt_data.items():
            if not raw_segs: continue
            if raw_segs[0][0] > max_pred_time: continue
            for s, e, lab in raw_segs:
                e = min(e, max_pred_time)
                if s >= e: continue
                for f in range(int(round(s * target_fps)), int(round(e * target_fps))):
                    if f < len(final_preds):
                        pred_lab = activity_names[final_preds[f]]
                        if lab in idx_map and pred_lab in idx_map:
                            cm[idx_map[lab]][idx_map[pred_lab]] += 1

        plot_confusion(cm, ALL_CLASSES,
                       OUTPUT_BASE_DIR / f"TOP{rank}_{run_id}_confusion.png")

    print(f"\nDONE. Results in {OUTPUT_BASE_DIR}")
    print(f"  - master_optimization_report.csv  ({total_combos} rows)")
    print(f"  - raw_inference_cache.xlsx")
    print(f"  - TOP1_<run>_confusion.png  ...  TOP5_<run>_confusion.png")
