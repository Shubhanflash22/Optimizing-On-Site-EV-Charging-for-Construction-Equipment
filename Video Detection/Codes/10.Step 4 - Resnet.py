...
VIDEO_PATH  = Path(r"/mnt/nvme1/avik_shubhan/resnet3d/day_3_pred/Day_3.mp4")
OUTPUT_DIR  = Path(r"/mnt/nvme1/avik_shubhan/resnet3d/day_3_pred")
...
def save_results(predictions, confidences, cycles, productivity, activity_names, fps, save_dir):
    ...
    # frame CSV
    with open(save_dir / "frame_predictions.csv_idling_included", 'w') as f:
        f.write("Frame,Time_s,Activity,Confidence\n")
        for i, (pred, conf) in enumerate(zip(predictions, confidences)):
            f.write(f"{i},{i/fps:.3f},{activity_names[pred]},{conf:.4f}\n")

    # cycles JSON
    with open(save_dir / "cycles.json_idling_included", 'w') as f:
    ...
    with open(save_dir / "summary.json_idling_included", 'w') as f:
