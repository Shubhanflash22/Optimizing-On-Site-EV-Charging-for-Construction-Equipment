"""
3D Action Recognition - TRIAL 6: Fix the two gaps Trial 5 left open
====================================================================
Trial 5 got baseline 63.14% -> final_combined 70.98%, but the per-epoch
logs exposed two problems that mean several numbers are undersold:

  (1) p3_flow_only and p3_two_stream NEVER got dedup_factor applied -
      they trained on the full 81%-overlap-redundant clip set (train_acc
      hit 96-99% by epoch 4-6 in both). dedup_factor=3 bought RGB
      +3.25pts in Trial 5 P1; it was never tested on flow/two-stream.
  (2) patience=3 cut several runs off right after their single best
      epoch (e.g. p3_flow_only's reported acc IS epoch 1 - it got WORSE
      every epoch after; final_combined's s42 seed stopped at epoch 4
      right after its only good epoch, while s43 ran to epoch 9 and
      kept climbing). patience=6 gives runs room to actually converge
      instead of reporting a lucky early checkpoint.

Trial 6 = same engine as Trial 5 (identical data/model/training code,
untouched), new experiment list:
  - re-baseline with dedup_factor=3 + patience=6 (cheap sanity anchor,
    confirms the Trial-5 dedup3 win holds under proper patience)
  - flow_only and two_stream, NOW with dedup_factor=3 + patience=6
    (the single highest-expected-value gap from Trial 5)
  - final_combined re-run with patience=6 (dedup3+erase+mixup+ce loss+
    two-stream+tta+3-seed ensemble - already Trial 5's best config,
    just letting it converge properly this time)
  - final_ensemble5: same recipe, 5 seeds instead of 3 (ensembling beat
    every individual seed's peak in Trial 5 - this pushes that lever
    further). Expensive - run last, only if the above justifies it.

Everything else (LOVO/fixed split logic, augmentation code, optical
flow, two-stream fusion, logit ensembling) is byte-for-byte Trial 5.

Outputs -> /data/shubhan_avik_work/Trial6/
  per-experiment checkpoints + histories, trial6_comparison.csv
  (the incremental "what worked" table), and trial6_study_summary.txt.
Resume-safe: completed experiments are skipped if their row already
exists in trial6_comparison.csv.
"""
...
TRIAL_DIR      = Path("/data/shubhan_avik_work/Trial6").resolve()
MODEL_SAVE_DIR = TRIAL_DIR
MODEL_SAVE_DIR.mkdir(parents=True, exist_ok=True)
COMPARISON_CSV = MODEL_SAVE_DIR / "trial6_comparison.csv"
...
# The locked baseline the whole study is measured against. Bakes in the two
# confirmed Trial-5 P1 winners (dedup3, ce over focal) plus patience=6 so
# runs get room to converge instead of reporting a lucky early checkpoint.
BASELINE = RecipeConfig(
    name="baseline_d3p6", phase="P0",
    dedup_factor=3, loss="ce", patience=6,
    desc="Trial-5 best single knobs locked in: dedup3 + ce loss + patience=6 "
         "(re-anchor point for Trial 6, comparable to Trial-5 baseline=63.14%)",
)


def make_experiments():
    """Trial 6: targeted re-runs of the two gaps Trial 5 left open, plus one
    push further on ensembling. Not a full OFAT grid - Trial 5 already
    covered that; this reuses its confirmed winners."""
    B = BASELINE
    exps = [B]

    # ---- Close gap #1: flow/two-stream never had dedup applied in Trial 5,
    #      so both were still training on the full redundant clip set
    #      (train_acc hit 96-99% by epoch 4-6). Now both get dedup3+patience6.
    exps += [
        dc_replace(B, name="p6_flow_dedup3", phase="P6",
                   stream="flow",
                   desc="Optical-flow-only, NOW with dedup3+patience6 "
                        "(Trial-5 p3_flow_only never had dedup applied)"),
        dc_replace(B, name="p6_two_stream_dedup3", phase="P6",
                   stream="two",
                   desc="RGB+flow two-stream, NOW with dedup3+patience6 "
                        "(Trial-5 p3_two_stream never had dedup applied)"),
    ]

    # ---- Close gap #2: re-run Trial 5's winning final_combined recipe with
    #      patience=6 instead of 3, so seeds that were cut off early (e.g.
    #      s42 stopped at epoch 4 right after its only good epoch) get room
    #      to actually converge like s43 did (ran to epoch 9, kept climbing).
    exps += [
        dc_replace(B, name="p6_final_combined", phase="P6",
                   aug_random_erase=True, mixup_alpha=0.2,
                   stream="two", tta=True, seeds=(42, 43, 44),
                   desc="Trial-5's best recipe (dedup3+erase+mixup+ce+"
                        "two-stream+tta, 3-seed ensemble), patience=6 "
                        "so no seed gets cut off mid-climb"),
    ]

    # ---- Push the strongest confirmed lever further: in Trial 5, the 3-seed
    #      ensemble beat every individual seed's peak accuracy on both
    #      p2_ensemble3 and final_combined. 5 seeds tests if that keeps
    #      paying off. Expensive - run this last, only if p6_final_combined
    #      justifies the extra cost.
    exps += [
        dc_replace(B, name="p6_final_ensemble5", phase="FINAL",
                   aug_random_erase=True, mixup_alpha=0.2,
                   stream="two", tta=True, seeds=(42, 43, 44, 45, 46),
                   desc="Same as p6_final_combined but 5-seed ensemble "
                        "instead of 3 (most expensive run in the study - "
                        "only justified if p6_final_combined confirms the "
                        "recipe is still improving with patience=6)"),
    ]
    return exps
...
def main():
    ...
    df.to_csv(MODEL_SAVE_DIR / "trial6_comparison_ranked.csv", index=False)

    show = ["name", "phase", "val_acc_mean", "val_acc_std", "delta_vs_baseline",
            "helped", "recall_loading", "minutes"]
    slog("")
    slog("=" * 100)
    slog(f"TRIAL 6 COMPARISON  (baseline = {base_acc:.2f}%)  sorted best-first")
    slog("=" * 100)
    slog(df[show].to_string(index=False))

    summary_path = MODEL_SAVE_DIR / "trial6_study_summary.txt"
    with open(summary_path, "w") as f:
        f.write("\n".join(SUMMARY_LINES) + "\n")
    print(f"\nComparison CSV -> {COMPARISON_CSV}")
    print(f"Ranked CSV     -> {MODEL_SAVE_DIR / 'trial6_comparison_ranked.csv'}")
