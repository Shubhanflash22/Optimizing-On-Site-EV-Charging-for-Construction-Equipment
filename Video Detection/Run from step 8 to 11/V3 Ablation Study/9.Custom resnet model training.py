...
# ============================
# Console summary logger (writes a clean .txt when training finishes)
# ============================
SUMMARY_LINES = []
...
BASE_DIR = Path("/data/shubhan_avik_work/Targeted_run_3").resolve()
DATASET_DIR = BASE_DIR / "Dataset_Ten_days"
...
# ============================
# 4. Data Preparation - Using pre-split train/val from Step 8
# ============================
print("\n" + "="*60)
print("Loading Pre-Split Dataset (train/ and val/ from Step 8)")
print("="*60)

def collect_clips(split_dir):
    """Collect all clip subdirectories from split_dir/class/ folders."""
    clips = []
    for label in ACTIVITY_NAMES:
        label_dir = split_dir / label
        if label_dir.exists():
            subdirs = [v for v in label_dir.iterdir() if v.is_dir()]
            clips.extend(subdirs)
        else:
            print(f"WARNING: {label_dir} not found")
    return clips

train_videos = collect_clips(TRAIN_DIR)
val_videos   = collect_clips(VAL_DIR)

print(f"Train clips found : {len(train_videos)}")
print(f"Val   clips found : {len(val_videos)}")
slog("STEP 9 — 3D ResNet Training")
slog(f"Train clips: {len(train_videos)}  |  Val clips: {len(val_videos)}")
slog(f"Config: clip={CLIP_LENGTH} crop={CROP_SIZE} fps={TARGET_FPS} "
     f"batch={BATCH_SIZE} lr={LEARNING_RATE} epochs={NUM_EPOCHS}")

# Compute mean/std from training set (no normalization yet)
stats_dataset = VideoClipDataset(
    TRAIN_DIR, ACTIVITY_NAMES, train_videos,
...
np.save(str(MODEL_SAVE_DIR / "dataset_mean.npy"), mean)
np.save(str(MODEL_SAVE_DIR / "dataset_std.npy"),  std)

mean = np.load(str(MODEL_SAVE_DIR / "dataset_mean.npy"))
std  = np.load(str(MODEL_SAVE_DIR / "dataset_std.npy"))

train_dataset = VideoClipDataset(
    TRAIN_DIR, ACTIVITY_NAMES, train_videos,
    CLIP_LENGTH, CROP_SIZE, STRIDE,
    train=True, normalize=True, mean=mean, std=std
)

val_dataset = VideoClipDataset(
    VAL_DIR, ACTIVITY_NAMES, val_videos,
    CLIP_LENGTH, CROP_SIZE, STRIDE,
    train=False, normalize=True, mean=mean, std=std
)
...
for epoch in range(NUM_EPOCHS):
    ...
    slog(f"Epoch {epoch+1}/{NUM_EPOCHS}: train_loss={train_loss:.4f} "
    ...
    if val_loss < best_val_loss:
        ...
        save_path = MODEL_SAVE_DIR / "resnet3d_best_ten_days.pth"
...
# ============================
slog("")
slog("TRAINING COMPLETE")
slog(f"Best Validation Loss : {best_val_loss:.4f}")
slog(f"Best Validation Acc  : {best_val_acc:.2f}%")
slog(f"Best model saved to  : {MODEL_SAVE_DIR / 'resnet3d_best_ten_days.pth'}")
summary_path = MODEL_SAVE_DIR / "step9_summary.txt"
